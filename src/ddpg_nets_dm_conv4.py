...
def theta_q(dimO, dimA, l1, l2):
    ...
    with tf.variable_scope("theta_q"):
        return [tf.Variable(fanin_init([dimO, l1]), name='1w_o'),
                ...
                tf.Variable(fanin_init([l1, l2]), name='4w_u'),
                tf.Variable(fanin_init([l2], dimO), name='4b_u'),
                tf.Variable(fanin_init([l1, dimA]), name='4w_yu'),
                tf.Variable(fanin_init([dimA], dimO), name='4b_yu'),
                tf.Variable(fanin_init([dimA, l2]), name='4w_a'),
                tf.Variable(fanin_init([l1,l1]), name='4w_zu'),
                tf.Variable(fanin_init([l1], dimO), name='4b_zu'),
                tf.Variable(fanin_init([l1, l2]), name='4w_conv'),
                tf.Variable(tf.random_uniform([l2, 1], -3e-4, 3e-4), name='5w_u'),
                tf.Variable(tf.random_uniform([1], -3e-4, 3e-4), name='5b_u'),
                tf.Variable(fanin_init([l2, dimA]), name='5w_yu'),
                tf.Variable(fanin_init([dimA], dimO), name='5b_yu'),
                tf.Variable(tf.random_uniform([dimA, 1],-3e-4, 3e-4), name='5w_a'),
                tf.Variable(fanin_init([l2,l2]), name='5w_zu'),
                tf.Variable(fanin_init([l2], dimO), name='5b_zu'),
                tf.Variable(tf.random_uniform([l2, 1],-3e-4, 3e-4), name='5w_conv')]
...
def qfunction(obs, act, theta, name="qfunction"):
    with tf.variable_op_scope([obs, act], name, name):
        ...
        h4 = tf.nn.relu(h4_u+ h4_a + h4_z)
        h5_u = tf.matmul(h4_u, theta[28]) + theta[29]
        h5_a = tf.matmul(tf.multiply(h0_a,tf.matmul(h4_u,theta[30])+theta[31]),theta[32])
        h5_z = tf.matmul(tf.multiply(h4,tf.matmul(h4_u,theta[33])+theta[34]),theta[35])
        qs = h5_u + h5_a + h5_z
        q = tf.squeeze(qs, [1], name='h5-q')
        return q
